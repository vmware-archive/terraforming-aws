data "template_file" "ops_manager" {
  ...
  vars {
    iam_instance_profile_arn = "${data.aws_iam_instance_profile.ops_manager.arn}"
    ops_manager_bucket_arn   = "${aws_s3_bucket.ops_manager_bucket.arn}"
    iam_ops_manager_role_arn = "${data.aws_iam_role.ops_manager.arn}"
  }
}
...
data "aws_iam_policy_document" "ops_manager" {
  ...
  statement {
    ...
    resources = [
      "${compact(concat(list(data.aws_iam_role.ops_manager.arn), var.additional_iam_roles_arn))}",
    ]
  }
}

...
#### Certs: Must select one of two options
1. Option 1
- ssl_cert: **(optional)** SSL certificate for HTTP load balancer configuration. Required if `ssl_ca_cert` is not specified.
- ssl_private_key: **(optional)** Private key for above SSL certificate. Required if `ssl_ca_cert` is not specified.
1. Option 2
